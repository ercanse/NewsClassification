"""
Trains a linear Support Vector Machine on the feature vectors and target values loaded from the database.
...
def evaluate_classifier_using_fixed_split(feature_vectors, target_values):
    """
    Evaluates the linear Support Vector Machine using a fixed 80-20 training-test split of the dataset.
    ...
    mnb = LinearSVC().fit(feature_vectors_train, target_values_train)
...
def evaluate_classifier_using_repeated_cross_validation(feature_vectors, target_values, n_folds=10, iterations=10):
    """
    Evaluates the linear Support Vector Machine using n-fold stratified cross-validation.
    ...
    for _ in xrange(iterations):
        ...
        scores.append(cross_val_score(LinearSVC(), feature_vectors, target_values, cv=k_fold).mean())
...
def train_classifier(feature_vectors, target_values):
    ...
    :param target_values: labels representing values for each feature vector
    :return: LinearSVC instance trained on the full dataset
    """
    return LinearSVC().fit(feature_vectors, target_values)
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description="Trains a linear Support Vector Machine using feature vectors from a given database.\n",
